...
public class OrderHistoryServiceImpl implements OrderHistoryService{
	...
	public OrderResponseDto placeOrder(List<PetDto> petDtoList,Long userId) {
		...
		List<Long> petIdList = petDtoList.stream()
				.map(this::validatePetId).collect(Collectors.toList());
		...
		List<OrderHistory> orderList = petIdList.stream().map(petId->saveOrder(petId,userId)).collect(Collectors.toList());
		...
		orderResponseDto.setOrderHistoryId(orderList.get(0).getOrderHistoryId());
	...
	private OrderHistory saveOrder(Long petId,Long userId)
	{
		Optional<User> userOptional = userDao.findByUserId(userId);
		if(!userOptional.isPresent())
			return null;
